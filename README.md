...
- **Vue d'ensemble** : un indice par ligne, avec la meilleure config sur la fenêtre courante, sa P(réussite), l'écart au hasard et la fraîcheur des données.
  ...
  - **Sélection multiple** (Ctrl+clic, Maj+clic) : avec deux configs ou plus, le graphique montre le **portefeuille** qu'elles forment (en couleur, les configs seules restant en gris), avec ses statistiques : rendement, DD max, Rend/DD, trades, challenge -30j, configs positives et DD moyen des configs seules (l'écart avec le DD du portefeuille mesure la diversification). Deux lectures au choix : **capital partagé** (parts égales, equity = moyenne des equities) ou **cumul** (chaque config sur tout le capital à son levier, gains et pertes s'additionnent). Les courbes sont lues en escalier sur l'union des barres des configs ; le DD suit la règle de la première config. La P(réussite) n'est pas agrégée : elle demande l'equity de toute la fenêtre, que le suivi ne garde pas.
  - **Groupes** : un groupe est un nom et un ensemble de configs (une config peut être dans plusieurs groupes), gardé dans la même base DuckDB (tables `groupes` et `groupe_configs`). Le sélecteur **Groupe** de la barre du suivi n'affiche que les configs du groupe et les sélectionne toutes : leur portefeuille apparaît aussitôt (les filtres symbole et timeframe repassent à « Tous »). Le menu **Groupe ▾** crée un groupe avec la sélection, ajoute la sélection à un groupe existant, la retire du groupe affiché, renomme ou supprime le groupe (ses configs restent sauvegardées). La colonne **Groupes** du tableau liste les groupes de chaque config. **Actualiser** ne rejoue que les configs affichées, donc celles du groupe.
